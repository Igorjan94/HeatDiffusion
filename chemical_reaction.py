...
def initial_X(space_steps):
    return [X_left] + [X_right for _ in range(space_steps - 1)]

def initial_T(space_steps):
    return [Tm] + [T0 for _ in range(space_steps - 1)]
...
def euler_forward_step(prev_X, prev_T):
...
def euler_backward_step(prev_X, prev_T):
    n = len(prev_X)
...
def refresh_parameters():
    global dt, dz, time_steps, space_steps, rate
    global R, E, K, alpha, Q, rho, T0, C, lam, D, kappa, time_steps, space_steps

    e2f, e2i = lambda x: float(x.get()), lambda x: int(x.get())
...
def show_plots(event):
    ...
    for mnum in map(int, listm.curselection()):
        c, method_step = colors[mnum], numerical_method_steps[mnum]
        ...
        for i in range(time_steps):
            if i % rate == 0:
                ...
                if drawX.get() == 1:
                    xx.plot(xs, ys, zxs, color=c)
                if drawT.get() == 1:
                    tx.plot(xs, ys, zts, color=c)
                if drawW.get() == 1:
                    wx.plot(xs, ys, zws, color=c)
...
def main():
    ...
    global listm, edt, edz, etau, el, erate
    ...
    labelm = Label(comp, text = 'Methods:')
    labelm.pack(side='top', padx=5, pady=5)
    optionsm = [
        'Explicit (bad)',
        'Partially implicit',
        'Partially implicit + diagonal',
        'Patially implicit with iterations',
#        'Схема с линеаризацией по Ньютону' <-- such third derivative, many computations, very hard to implement, wow
    ]
    listm = Listbox(comp, width=40, height=6, activestyle='none', selectmode='multiple')
    for item in optionsm:
        listm.insert(END, item)
    listm.pack(side = 'top', padx=10)
    listm.select_set(0)
    ...
    plot = Button(phys, text = 'показать графики')
